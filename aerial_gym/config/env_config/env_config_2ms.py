...
class EnvCfg2Ms:
    class env:
        ...
        num_physics_steps_per_env_step_mean = 5  # 每个环境步骤之间的平均物理步数
        num_physics_steps_per_env_step_std = 0  # 每个环境步骤之间的物理步数标准差
        render_viewer_every_n_steps = 2  # 每n步渲染一次观察者
        ...
        sample_timestep_for_latency = True  # 为延迟噪声采样时间步
        perturb_observations = True  # 扰动观测值
        keep_same_env_for_num_episodes = 1  # 保持相同环境的回合数
        write_to_sim_at_every_timestep = False  # 再次确认是否在每个时间步写入模拟数据
